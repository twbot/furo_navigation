...
void pathFind(int startX, int startY,
                        int endX, int endY)
{
        static std::priority_queue<node> queue[2]; // list of open (not-yet-tried) nodes
        static int index; // queue index
        static node* n0;
        static node* m0;
        ...
        while(!queue[index].empty())
        {
            ...
            n0=new node( queue[index].top().getX(), queue[index].top().getY(),
            ...
            for(i=0;i<dir;i++)
            {
                    ...
                    || closed_nodes_map[xdx][ydy]==1))
                {
                    ...
                    else if(open_nodes_map[xdx][ydy]>m0->getF())
                    {
                        ...
                        // replace the node
                        // by emptying one queue to the other one
                        // except the node to be replaced will be ignored
                        // and the new node will be pushed in instead
                        while(!(queue[index].top().getX()==xdx &&
                               queue[index].top().getY()==ydy))
                        {
                        ...
                        index=1-index;
                        queue[index].push(*m0); // add the better node instead
                    }
